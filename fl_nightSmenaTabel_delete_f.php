...
//fl_nightSmenaTabel_delete_f.php
    ...
    if (empty($_SESSION['login']) || empty($_SESSION['id'])){
        ...
        if ($_POST){
            ...
            if (!isset($_POST['tabel_id'])){
                ...
                $query = "DELETE FROM `fl_journal_tabel_nightsmens` WHERE `tabel_id` = '{$_POST['tabel_id']}'";
                            
                $res = mysqli_query($msql_cnnct, $query) or die(mysqli_error($msql_cnnct) . ' -> ' . $query);

                //Обновим и табель
                $query = "UPDATE `fl_journal_tabels` SET `night_smena` = '0' WHERE `id`='{$_POST['tabel_id']}';";
