	...
	(function(G){
		...
		G.fn.maphilight=function(O){
			...
			return this.each(function(){
				...
				R=G("<div></div>").css({display:"block",background:'url("'+this.src+'")',position:"relative",padding:0,width:this.width,height:this.height});
				if(Y.wrapClass){
					if(Y.wrapClass===true){
				...
				G(Q).bind("alwaysOn.maphilight",function(){
					...
					G(Q).find("area[coords]").each(function(){
						...
						var colorss = document.getElementById("tt1").value;
						document.getElementById('xy').innerHTML = colorss;
						var Z,a;
						a=L(this,Y);
						if(a.alwaysOn){
							if(!V&&J){
				...
				G(Q).trigger("alwaysOn.maphilight").find("area[coords]").bind("mouseover.maphilight",X).bind("mouseout.maphilight",function(Z){N(T)});
				U.before(T);
